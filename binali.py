#!/usr/bin/python
# -*- coding: UTF-8 -*-

QUOTES = (
    "Bulut diye birşey çıkarmışlar, herkes içine birşey atıyor.",
    "Bu bilişim fazla kafa yorursan sıyırırsın... kullanacaksın, nimetlerinden kullanıp, yararlanıp işini göreceksin, kafayı taktın mı hıhı. o zaman işin kötü. çok fazla, hikmetine fazla şey yapmamak lazım.",
    "binali in da sky with diamonds: https://vimeo.com/31575968",
    "Sonra bahçesinde gençler kızlı, erkekli oturuyor. Ben çok şaşırdım. Burada yoldan çıkarım dedim. Benim okumam lazımdı.",
    "5 dakika 30 kuruş devam etsin. Ama bir de 2 dakika 10 kuruş, olursa bu iş tadından yenmez. Benimki sadece fikir.",
    "(PTT Genel Müdürü'ne) Bırak bunu yemezler, hayır işini vakıflar yapar, [siz] ticarethanesiniz.",
    "(Pegasus Havayollarının uygulamalarını yorumlarken) Oturduk para, kalktık para, su içtik para, bedava hiçbir hizmet yok. Bundan şaşmayın bu doğru.",
)

import random

print random.choice(QUOTES)
